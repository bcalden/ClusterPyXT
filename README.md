# ClusterPyXT-dev
This repository contains the development version of ClusterPyXT, a Python package for the analysis of X-ray galaxy cluster observations. The stable version of ClusterPyXT can be found on the master branch of this repository.

This is a work in progress and the code is not complete yet as it is in the early stages of development. The goal is to be completely commented, documented, easier to follow, and more efficient than the current version of ClusterPyXT. Feel free to contribute.
...
After following the instructions above, go the `ClusterPyXT` directory and run `python -m cpxt` to initialize the system configuration. 
...
Create a new galaxy cluster project in the GUI with the new cluster button.

### Processing a Cluster
After a cluster is initialized, you can simply start/continue the pipeline stage by stage in the GUI. 
...
After this region file is created, continue running ClusterPyXT by clicking the `Stage 4` button in the GUI.
...
This stage only requires all previous stages to be completed. Stage 5 calculates the adaptive circular bins, generates the scale map, and calculates exposure corrections. While, it can take a long time (~10s of hours) it is usually done within a half hour or less depending on your computer speed. 
